          ...
          <el-table-column v-else-if="item.type == 'opera'" :prop="item.key" :label="item.label" :width="item.width" align="center">
              <template scope="scope">
                <div class="opera-menu">
                  ...
                  <a :href="'/#/saleFreightProvince?freightId='+scope.row.id+'&name='+scope.row.name">
                    <el-button  size="small">设置邮费</el-button>
                  </a>
...
<script>
  ...
  export default{
    ...
    methods:{
      ...mapActions(['setViewImage','setSearchParams','getSaleFreightTempListPage','setStartToast','saveSaleFreightTemp']),
      initView(){
      ...
      searchList(params){
        this.loading = true
        this.getSaleFreightTempListPage(params)
          .then((rs)=>{
      ...
      submitCateMsgModel(){
        ...
        this.saveSaleFreightTemp(_opts)
          .then(()=>{
