	...
	<div>
	    ...
	    <div class="section-box">
              ...
              <div class="block-box base-box">
                ...
                <el-select filterable size="small" v-model="selectedBelong" placeholder="请选择" @change="sellerMainIdChange" v-else>
              ...
              <div class="block-box base-box" v-for="(item,i) in mapShipAreaValue" v-if="mapShipAreaValue.length>0">
                ...
                <div class="block-box base-box">
                  ...
                  <el-select :disabled="selectedBelong.id ? false : true" v-if="selectedBelong.extra == 1" filterable clearable remote :remote-method="remoteMethodSaleBrand"  v-model="saveProductEdit.brandId" class="input-item-middle input-item-large" size="small"  placeholder="请选择品牌">
                  ...
                  <el-select :disabled="selectedBelong.id ? false : true" v-else filterable clearable v-model="saveProductEdit.brandId" class="input-item-middle input-item-large" size="small"  placeholder="请选择品牌">
                    <template v-for="subItem in saleBrandNameList">
                      <el-option :key="subItem.code" :label="subItem.name" :value="subItem.code">
                        {{subItem.name}}
                      </el-option>
                  ...
                  <el-button size="small" :style="{'marginLeft':'10px','padding':'6px 20px','width':'150px','vertical-align':'middle'}" v-if="editType == 'edit'" @click="costPriceChangeRecord" type="primary" >成本价调价记录</el-button>
		          	...
		          	<div class="item-table">
			            <el-table :data="goodsSkuConfig" style="width: 100%;" >
			              ...
			              <el-table-column align="center" label="采购商品ID" width="120" v-if="selectedBelong.extra==1 && shipAreaValue.length == 1 && mapArrToObj[shipAreaValue] && mapArrToObj[shipAreaValue].outWMS > 0">
						    	    <template scope="scope">
                    ...
                    <el-table-column prop="grossWeight" align="center" label="毛重(g)" width="120">
                      <template scope="scope">
                        <span class="cell-edit-input"><el-input size="small" class="input-item-middle" :disabled="(selectedBelong.extra==1 && shipAreaValue.length == 1 && mapArrToObj[shipAreaValue] && mapArrToObj[shipAreaValue].outWMS > 0)?true:false" v-model="scope.row.grossWeight"></el-input></span>
                    </template>
                    </el-table-column>
                    ...
                    <template v-if="shipAreaOptions.length>0" v-for="(item,i) in shipAreaValue">
                      <el-table-column prop="code" align="center" :label="'发货编码('+shipAreaOptions.filter((obj)=>obj.deliverAreaId==item)[0].deliverAreaName+')'" width="120" :key="i" >
                        <template scope="scope">
								          <span class="cell-edit-input">
                            <el-input size="small" class="input-item-middle"
                                    :disabled="(selectedBelong.extra==1 && shipAreaValue.length == 1 && mapArrToObj[shipAreaValue] && mapArrToObj[shipAreaValue].outWMS > 0)?true:false"
                                    v-model="scope.row['code'+i]"
        ...
        if(this.editType == 'edit'){
          ...
          this.callGetProductDetail()
            ...
            .then(()=>{
                ...
                console.log(_this.shipAreaOptions)
...
//        console.log(obj)
	    	...
	    	this.shipAreaPromise(obj.id).then((rs)=>{
              ...
              //商品下的 发货地 合并商家下的 发货地
              //入仓发货地 合并 到 发货地列表
              this.storageAreaOptions = this.goodsDetailFetch.rs.skuDeliverAreas
              console.log(this.goodsDetailFetch.rs.skuDeliverAreas)
		    			this.goodsDetailFetch.rs.deliverAreas.map((data1,i)=>{
                rs.filter((data2)=>data1.deliverAreaId == data2.deliverAreaId).length == 0 ?
