  ...
  <div>
    ...
    <div class="section-box">
      ...
      <template v-if="editStep == 2">
          ...
          <!--<div class="block-box base-box label-float-layout">-->
            ...
            <!--<div class="inline-box">-->
              <!--<div class="mg-b " style="margin-left: 4px" v-for="(item,index) in [1,2,3]">-->
                ...
                <!--</el-cascader>-->
              <!--</div>-->
              <!--&lt;!&ndash;<el-button @click="showFrontCateSelect">显示</el-button>&ndash;&gt;-->
            <!--</div>-->
          <!--</div>-->
                  ...
                  <template v-else-if="item.inputType == 2">
                    <div class="inline-box">
                      ...
                      <template v-if="baseGoodsId && baseSyncConfig.isSyncKeyProperty == 1">
                        <el-radio class="mg-b-h" :disabled="true" v-model="item.selected"  :label="item.inputValue"><el-input v-model.trim="item.inputValue" @blur="checkPvName(item,item.inputValue)" class="input-item-middle" size="small"></el-input></el-radio>
                      </template>
                      <template v-else>
                    ...
                    <div class="inline-box">
                      <el-checkbox-group v-model="item.selected">
                        ...
                        <template v-for="(subItem,sIdx) in item.mulOptions">
                          <template v-if="baseGoodsId && baseSyncConfig.isSyncKeyProperty == 1">
                            <el-checkbox  class="mg-b-h" :disabled="true" @change="mulSelect(idx,$event,sIdx)" :label="subItem.pvName"><el-input v-model.trim="subItem.pvName" @blur="checkPvName(item,subItem.pvName,sIdx)" class="input-item-middle" size="small"></el-input></el-checkbox>
                          </template>
                          <template v-else>
        ...
        <div class="item-box">
          <div class="mg-b">
            <div class="sub-title-box inline-box">
              <span>销售单元</span>
            </div>
            <div class="inline-box" style="height: 30px;line-height: 30px;vertical-align: 9px">
              <div class="inline-box">
                <span style="margin: 0 5px  0 20px;">发货地：</span>
                <template v-if="baseGoodsId && editType == 'new'">
                  <el-select clearable :disabled="editType == 'edit' ? true : false" @change="callBaseAreaTempChange" v-model="baseGoodsDeliverArea.deliverArea.deliverAreaId" class="input-item-middle input-item-large-select" size="small">
                    <template v-for="subItem in baseGoodsDeliverArea.options">
                      <el-option v-if="subItem.status == 1" :label="subItem.label" :value="subItem.value"></el-option>
                    </template>
                  </el-select>
                  <span v-if="sellerFeeMap[baseGoodsDeliverArea.deliverArea.deliverAreaId]">
                    <span style="margin: 0 5px;">每订单运费平均值：{{sellerFeeMap[baseGoodsDeliverArea.deliverArea.deliverAreaId].perOrderSellerFreight}}</span>
                    每SKU运费平均值：{{sellerFeeMap[baseGoodsDeliverArea.deliverArea.deliverAreaId].perSkuSellerFreight}}
                </span>

                </template>
                <template v-else>
                  <el-select clearable :disabled="editType == 'edit' ? true : false" @change="callSelectFreight" v-model="saveProductEdit.deliverArea.deliverAreaId" class="input-item-middle input-item-large-select" size="small">
                    <template v-for="subItem in sellerDeliver.options">
                      <el-option :label="subItem.label" :value="subItem.value"></el-option>
                    </template>
                  </el-select>
                  <span v-if="sellerFeeMap[saveProductEdit.deliverArea.deliverAreaId]">
                    <span style="margin: 0 5px;">每订单运费平均值：{{sellerFeeMap[saveProductEdit.deliverArea.deliverAreaId].perOrderSellerFreight}}</span>
                    每SKU运费平均值：{{sellerFeeMap[saveProductEdit.deliverArea.deliverAreaId].perSkuSellerFreight}}
                </span>
                </template>
              </div>
            </div>
          ...
          <div class="multiple-box" >
            <div class="multiple-wrap">
              ...
              <div v-if="baseGoodsId" style="width: 90px;margin-right: 104px">
                <el-input style="width: 90px"  class="input-item-middle"  size="small" v-model.trim="multipleConfig.price" @blur="multipleBlur(multipleConfig.price,'price')" placeholder="单位销售价"></el-input>

              </div>
              <div style="width: 140px">
                <el-input style="width: 100px"  class="input-item-middle" size="small" v-model.trim="multipleConfig.restriction" @blur="multipleBlur(multipleConfig.restriction,'restriction')" placeholder="限购数量"></el-input>
              </div>
              <div style="width: 140px">
                <el-input style="width: 100px"  class="input-item-middle" size="small" v-model.trim="multipleConfig.dailyActivityPrice" @blur="multipleBlur(multipleConfig.dailyActivityPrice,'dailyActivityPrice')" placeholder="日常活动价"></el-input>
              </div>
              <div style="width: 140px">
                <el-input style="width: 100px"  class="input-item-middle" size="small" v-model.trim="multipleConfig.promotionActivityPrice" @blur="multipleBlur(multipleConfig.promotionActivityPrice,'promotionActivityPrice')" placeholder="大促活动价"></el-input>
              </div>
            </div>
          </div>
          ...
          <div v-if="baseGoodsId" class="item-table">
            <el-table ref="multipleTable" :data="goodsSkuConfig" style="width: 100%;" @selection-change="skuUnitBatch">
              ...
              <el-table-column v-if="baseSaleUnitReady" prop="status" label="销售状态" align="center">
              ...
              <el-table-column  v-if="baseSaleUnitReady" prop="skuName"  label="销售单元"  align="center">
                <template scope="scope">
                  <span class="key-item">{{scope.row.skuName | changeSkuCode}}</span>
                </template>
              </el-table-column>
              <el-table-column  v-if="baseSaleUnitReady" prop="skuCode" width="180" label="关联sku"  align="center">
              ...
              <el-table-column v-if="baseSaleUnitReady" prop="price" label="单位销售价" width="100" align="center">
                <template scope="scope" >
                  <el-input v-model="scope.row.price" @change="baseSkuRate(scope.row.price,scope.row)" size="small" class="input-item-middle" style="width: 90px"></el-input>
                </template>
              </el-table-column>
              <el-table-column  v-if="baseSaleUnitReady" prop="price" label="总销售价" width="100"  align="center">
                <template scope="scope" >
                  <span class="key-item">{{Number(scope.row.count) ? accMul(scope.row.price,scope.row.count) || scope.row.price : ''}} </span>
                </template>
              </el-table-column>
              <el-table-column  v-if="baseSaleUnitReady" label="限购数量"  align="center" width="140">
                <template scope="scope">
                  <el-input v-model="scope.row.restriction"  size="small"  class="input-item-middle" style="width: 100px;"></el-input>
                </template>
              </el-table-column>
              <el-table-column  v-if="baseSaleUnitReady" label="日常活动价" align="center"  width="140">
                <template scope="scope">
                  <el-input v-model="scope.row.dailyActivityPrice"  size="small"  class="input-item-middle" style="width: 100px;"></el-input>
                </template>
              </el-table-column>
              <el-table-column  v-if="baseSaleUnitReady" label="大促活动价" align="center"  width="140">
                <template scope="scope">
                  <el-input v-model="scope.row.promotionActivityPrice"  size="small"  class="input-item-middle" style="width: 100px;"></el-input>
                </template>
              </el-table-column>
          ...
          <div v-else class="item-table">
            <el-table  :data="goodsSkuConfig" style="width: 100%;" >
              ...
              <el-table-column  label="限购数量"  align="center" width="140">
                <template scope="scope">
                  <el-input v-model="scope.row.restriction"  size="small"  class="input-item-middle" style="width: 100px;"></el-input>
                </template>
              </el-table-column>
              <el-table-column  label="日常活动价" align="center" width="140">
                <template scope="scope">
                  <el-input v-model="scope.row.dailyActivityPrice"  size="small"  class="input-item-middle" style="width: 100px;"></el-input>
                </template>
              </el-table-column>
              <el-table-column  label="大促活动价" align="center"  width="140">
                <template scope="scope">
                  <el-input v-model="scope.row.promotionActivityPrice"  size="small"  class="input-item-middle" style="width: 100px;"></el-input>
                </template>
              </el-table-column>
        ...
        <div class="item-box" v-if="mapChannelRate[saveProductEdit.channel]">
          <div class="sub-title-box mg-b">
            佣金设置
          </div>
          <div class="multiple-box" v-if="saveProductEdit.channel == 2">
            <div class="multiple-wrap" style="right: 912px">
              <div style="width: 100px">
                批量设置：
              </div>
              <div style="width: 140px">
                <el-input style="width: 100px"  class="input-item-middle" size="small" v-model.trim="multipleConfig.totalProfit" @blur="multipleBlur(multipleConfig.totalProfit,'totalProfit')" placeholder="总分出利润"></el-input>
              </div>
            </div>
          </div>
          <div v-if="baseGoodsId" class="item-table">
            <el-table :data="skuUnitBatchList" style="width: 100%;" >
              ...
              <el-table-column  prop="price" label="单位销售价" width="90" align="center">
                <template scope="scope" >
                  <span class="key-item">{{scope.row.price}} </span>

                </template>
              </el-table-column>
              <el-table-column  prop="price" label="总销售价"  align="center">
                <template scope="scope" >
                  <span class="key-item">{{Number(scope.row.count) ? accMul(scope.row.price,scope.row.count) || scope.row.price : ''}} </span>
                </template>
              </el-table-column>
              <template v-if="baseSaleUnitReady">
                <template v-for="(rate,rateKey) in mapChannelRate[saveProductEdit.channel]">
                  <el-table-column v-if="rate.type == 'radio'" :label="rate.label" width="300" align="center">
                    <template scope="scope">
                      <el-radio class="radio" v-model="scope.row[channelRateKey[saveProductEdit.channel]][rate.key]" :label="0">常规分佣</el-radio>
                      <el-radio class="radio" v-model="scope.row[channelRateKey[saveProductEdit.channel]][rate.key]" :label="1">微商品分佣</el-radio>
                    </template>
                  </el-table-column>
                  <el-table-column v-else-if="rate.type == 'input'" :label="rate.label" width="150"  align="center">
                    <template scope="scope">
                      <el-input v-model="scope.row[channelRateKey[saveProductEdit.channel]][rate.key]" @change="planRate(scope.row,rate.key,scope.row[channelRateKey[saveProductEdit.channel]][rate.key])" size="small"  class="input-item-middle" style="width: 100px;"></el-input>
                    </template>
                  </el-table-column>
                  <el-table-column v-else :label="rate.label"  align="center"  width="150">
                    <template scope="scope">
                      <el-input disabled v-model="scope.row[channelRateKey[saveProductEdit.channel]][rate.key]" size="small"  class="input-item-middle" style="width: 100px;"></el-input>
                    </template>
                  </el-table-column>
                </template>
              </template>
...
//          value: 'label',
      ...
      sellerFeeMap:{
        // sellerId:{
        //   perOrderSellerFreight:0,
        //   perSkuSellerFreight:0
        // }
      },
      sellerDoubleChangeTime:{
        // sellerId:'Time'
      },
    }),
    methods:{
      ...
      'getSellerFee']),
      initView(){
        this.callBaseAreaTempChange = this.debounce((id)=>{
          this.baseAreaTempChange(id)
        },300)
        this.callSelectFreight = this.debounce((id)=>{
          this.selectFreight(id)
        },300)
        //初始化
        //获取类目
      ...
      multipleBlur(value,key){
        ...
        this.goodsSkuConfig.map((item)=>{
          if (key == 'totalProfit') {
            item['saleUnitHqbs'][key] = value
            this.planRate(item, key, value)
          }
          else if (key == 'price') {
            item[key] = value
            this.baseSkuRate(item[key], item)
          } else {
            item[key] = value
          }
        })
...
//        育成经理利润：A*0.018
      ...
      planMSRate(rs){
//        高级代理利润A：初级代理价>400 ? 100/初级代理价 再向下取整 : 25
//        育成代理利润B：A*0
//        经理利润C；A
//        育成经理利润D：A*0.15
//        总监利润：A*0.5
//        育成经理利润：A*0.1
        ...
        const _base = rs.saleUnitMsms.agentPrice > 400 ? 100/rs.saleUnitMsms.agentPrice * 100 : 25
        _rule.map((item)=>{
          let _result = _base * item.base,
            _num = _result.toFixed(4),
            _rNum = _num.replace(/\.?0+$/,'')

          rs.saleUnitMsms[item.key] = _rNum
        })
      ...
      baseSkuRate(price,rs){
        ...
        rs.dailyActivityPrice = this.accMul(rs.price,rs.count) ? this.accMul(rs.price,rs.count) : ''
        rs.promotionActivityPrice = this.accMul(rs.price,rs.count) ? this.accMul(rs.price,rs.count) : ''
...
//                price:this.getRowArr(item.unitSKUs,5,1),
                ...
                dailyActivityPrice:item.dailyActivityPrice/100,
                promotionActivityPrice:item.promotionActivityPrice/100,
              }
              if(this.channelRateKey[this.saveProductEdit.channel]){
                let _rate = {}
                ...
                } else if(this.saveProductEdit.channel == 8)(
                  _rate.saleUnitMsms.agentPrice = _rate.saleUnitMsms.agentPrice / 100
                )
                _opts = Object.assign({},_opts,_rate)
              }
...
//        }
      ...
      selectableSKU(row){
        let _ableArea = false
        if(this.editType == 'edit' && this.baseGoodsId){
          let _list =  row.unitSKUs[0].deliverAreas
          let _check = false
          _list.map((item)=>{
            if(item.deliverAreaId == this.saveProductEdit.deliverArea.deliverAreaId){
              _check = true
            }
          })
          if(!_check){
      ...
      skuUnitBatch(list){
        ...
        let _mapIdList = {}
        list.map((item)=>{
          item.unitSKUs[0].deliverAreas.map((area)=>{
            if(_mapIdList[area.deliverAreaId]){
              _mapIdList[area.deliverAreaId] += 1
            }  else {
              _mapIdList[area.deliverAreaId] = 1
            }
          })
        ...
        for(let i in _mapIdList){
            if(_mapIdList[i] == _listLength){
              _idList.push(i)
            }
        }
        if(this.baseGoodsId && this.editType == 'new'){
          this.skuUnitMapDeliver(_idList)
        }
      ...
      //快捷创建sku勾选发货地 列表
      skuUnitMapDeliver(list){
        let rs =  this.baseDetailFetch.skuDeliverAreas,
            _map = {}
        this.baseGoodsDeliverArea.deliverArea.deliverAreaId = ''
        this.baseGoodsDeliverArea.options = []
        this.baseGoodsDeliverArea.templateOpts = []
        rs.map((item)=>{
          if(!_map[item.deliverAreaId] && list.includes(item.deliverAreaId+'')){
            _map[item.deliverAreaId] = item.deliverAreaId
            let _opt = {
              label:item.deliverAreaName,
              value:item.deliverAreaId,
              status:item.deliverAreaStatus
            }
            this.baseGoodsDeliverArea.mapAreaTempId[item.deliverAreaId] = item.deliverAreaTemplateId ? item.deliverAreaTemplateId : ''
...
//          _this.baseGoodsDeliverArea.mapAreaTempId[item.deliverAreaId] = item.deliverAreaTemplateId
            this.baseGoodsDeliverArea.options.push(_opt)
            if(item.deliverAreaTemplateId){
              let _tempOpt = {
                label:item.deliverAreaTemplateName,
                value:item.deliverAreaTemplateId
              }
              this.baseGoodsDeliverArea.templateOpts.push(_tempOpt)
            }
          }
        ...
        this.initSellerDeliver.deliverAreaIds = _dList
        this.initDeliverAreasTemp(this.initSellerDeliver.deliverAreaIds)
      },
      //同步SKU图片
      mapSKUImage(){
...
////                  console.log(unit)
      ...
      //快捷创建运费模板修改 快捷创建新建
      baseAreaTempChange(id){
        this.callGetSellerFee(id)
      ...
      //发货地运费
      callGetSellerFee(deliverAreaId){
        if(deliverAreaId && !this.sellerFeeMap[deliverAreaId]){
          this.getSellerFee({deliverAreaId})
            .then((rs)=>{
              this.$set(this.sellerFeeMap,deliverAreaId,rs)
              // this.sellerFeeMap[deliverAreaId] = rs
            })
        }
      },
      //快捷创建详情初始化
...
////        console.log('===获取详情===')
      ...
      initDetail(rs,resolve){
        ...
        _this.getSellerArea({deliverAreaIds:this.initSellerDeliver.deliverAreaIds})
          .then((rs)=>{
...
//        _this.setProductUpload({detail:rs.detail.detail,images:rs.saleGoodsImages})
        _this.setProductUpload({detail:this.convertDetail(rs.detail.detail,2),images:rs.saleGoodsImages,detailVideos:rs.detailVideos || [],imagesVideo:rs.imagesVideo || []})
      ...
      initDeliverAreas(id){
        this.sellerDeliver.areaTempOpts = []
        this.getGoodsSellerMainDeliver({sellerMainId:id ? id : this.initSellerDeliver.sellerMainId})
          .then((rs)=>{
            rs.map((item)=>{
              let _opt = {
                label:item.name,
      ...
      selectFreight(id){
        ...
        this.callGetSellerFee(id)
        let _id = 0
        this.sellerDeliver.options.map((item)=>{
            if(item.value == id){
              _id = item.sellerMainId
            }
        })
        if(!_id || this.editType == 'edit') return
        this.initDeliverAreas(_id)
        this.saveProductEdit.deliverArea.sellerDeliverAreaTemplateId = ''
        return false
...
//          console.log(this.initSellerDeliver.deliverAreaIds)
          ...
          if(!this.initSellerDeliver.sellerMainId && mainId){
...
//          console.log(_check_s)
        ...
        this.goodsSkuConfig.map((item,idx)=>{
          ...
          if(this.saveProductEdit.channel == 8){
            ...
            this.planMSRate(item)
          }
          item.dailyActivityPrice = this.getRowArr(item.unitSKUs,6,1)
          item.promotionActivityPrice = this.getRowArr(item.unitSKUs,6,1)
...
//          console.log(typeof item.count)
          ...
          if(this.saveProductEdit.channel == 8)(
            item.saleUnitMsms.agentPrice = this.accMul(item.saleUnitMsms.agentPrice,100)
          )
...
//          }
          ...
          if(!(item.restriction+'')){
            this.getToast('请填写销售单元'+item.skuName.replace(this.skuSplitStrReg,'-')+'限购件数')
            return false
          }
          if(!(item.restriction+'').match(RegExpType.Num)){
            this.getToast('销售单元'+item.skuName.replace(this.skuSplitStrReg,'-')+'限购件数格式不正确')
            return false
          }
          if(!(item.dailyActivityPrice+'')){
            this.getToast('请填写销售单元'+item.skuName.replace(this.skuSplitStrReg,'-')+'日常活动价')
            return false
          }
          if(!(item.dailyActivityPrice+'').match(RegExpType.Price)){
            this.getToast('销售单元'+item.skuName.replace(this.skuSplitStrReg,'-')+'日常活动价格式不正确')
            return false
          }
          if(!(item.promotionActivityPrice+'')){
            this.getToast('请填写销售单元'+item.skuName.replace(this.skuSplitStrReg,'-')+'大促活动价')
            return false
          }
          if(!(item.promotionActivityPrice+'').match(RegExpType.Price)){
            this.getToast('销售单元'+item.skuName.replace(this.skuSplitStrReg,'-')+'大促活动价格式不正确')
            return false
          }
          ...
          item.dailyActivityPrice = this.accMul(item.dailyActivityPrice,100)
          item.promotionActivityPrice = this.accMul(item.promotionActivityPrice,100)
          if(this.baseGoodsId){
            item.unitSKUs.map((subItem)=>{
...
//        || item.propertyType == 2
          ...
          if(!this.baseGoodsId && item.propertyType == 2 && item.selected.length > 29){
...
//        opts.deliverArea.freightTemplateId = 1
